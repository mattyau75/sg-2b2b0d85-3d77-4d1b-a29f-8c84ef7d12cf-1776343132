  ...
  return (
    <>
      <div className={cn(
        ...
        (!forceVisible && errors.length === 0 && !isOpen) && "translate-y-20 opacity-0 pointer-events-none"
      )}>
        <Button
          onClick={onToggle}
          variant={errors.length > 0 ? "destructive" : "secondary"}
          className="gap-2 shadow-2xl border-2 border-white/10 h-11 px-5 rounded-full"
        >
          <AlertTriangle className={cn("h-4 w-4", errors.length > 0 && "animate-pulse")} />
          <span className="text-xs font-black uppercase tracking-widest">System Logs</span>
          {errors.length > 0 && (
            <Badge variant="secondary" className="bg-white text-destructive font-bold px-1.5 h-5 min-w-[1.25rem] rounded-full">
              {errors.length}
            </Badge>
          )}
        </Button>
